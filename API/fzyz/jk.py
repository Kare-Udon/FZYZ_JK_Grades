...
def send(request):
    ...
    #发送POST请求
    response = requests.post(url=url_login, headers=headers_autho, data=json.dumps(data), cookies=cookie)
    #保存获取到的含有autho的json
    ...
    #构造访问请求
    response = requests.get(url=url_studentId, headers=headers_studentId, cookies=cookie)
    #获取studentId
    ...
    #成绩查询地址构建
    school_id = '2246' #可拓展项
    url_score = 'https://api.fclassroom.com/ud-api-student/api/v1/exam/list?examSubjectValue=' + str(subject) + '&offset=1&pageSize=999&schoolId='+ str(school_id) +'&studentId=' + str(studentId)
    #构建成绩查询头
    headers_score = {
        'Client-Value': '22' ,
        'Connection': 'keep-alive',
    ...
    #格式化json
    str_score = json.dumps(json_score, ensure_ascii=False, sort_keys=True, indent=2)
    #暂时直接输出json
